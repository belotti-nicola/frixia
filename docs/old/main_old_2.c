...
void *adder_tcp(void *cast_me_to_ctx)
{
    ...
    fepoll->fepoll_handlers[reply] = *sv_create_callback(
        tmp->function,
        tmp->auxiliary
    );
...
void *main_loop(void *th_arg)
{
    ...
    while(*keep_looping)
    {
        ...
        for(int i=0;i<n;i++)
        {
            ...
            void *anything = fepoll->fepoll_handlers[fd].auxiliary;
            cb_ctx_t ctx = 
            {
                .cb_fd = fd,
                .cb_event_mask = mask,
            ...
            void *(*f)(void *) = (void *(*)(void *))fepoll->fepoll_handlers[fd].function;
            void   *a          = (void *)&ctx;
            if ( f == NULL )
            {
                printf("NULL f, %d\n",__LINE__);
...
void *stop_frixia(void *arg)
{
    ...
    fepoll_th_data_t *data = ctx->env->fepoll_ctx;
...
void *fepoll_context_counter(void *arg)
{
    ...
    *c = *c+1;
    ...
    printf("fepoll_context_counter %d\n",*c);
    return NULL;
}
